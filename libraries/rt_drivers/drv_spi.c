...
static rt_ssize_t xfer_my(struct rt_spi_device *device, struct rt_spi_message *message, rt_sem_t sem)
{

#define DMA_TRANS_MIN_LEN 10 /* only buffer length >= DMA_TRANS_MIN_LEN will use DMA mode */

    flag_status state = SET;
    ...
    while (message_length)
    {
        ...
        if ((instance->config->spi_dma_flag & RT_DEVICE_FLAG_DMA_TX) && (send_length > DMA_TRANS_MIN_LEN))
        {
            if (RT_IS_ALIGN((rt_uint32_t)send_buf, 4))
            {
        ...
        if (message->send_buf && message->recv_buf)
        {
                ...
                (send_length >= DMA_TRANS_MIN_LEN))
            {
                _spi_dma_receive(instance, (uint8_t *)recv_buf, send_length);
                _spi_dma_transmit(instance, (uint8_t *)send_buf, send_length);
                state = instance->config->dma_tx->dma_done & instance->config->dma_rx->dma_done & spi_i2s_flag_get(instance->config->spi_x, SPI_I2S_BF_FLAG);
